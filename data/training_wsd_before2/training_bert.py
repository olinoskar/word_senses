...
def train_word(folder, bert_model, bert_tokenizer):
	...
	try:
		...
		best = fmin(lambda args: hyperopt(args, X.clone(), y.clone(), new_dict), space, algo=tpe.suggest, max_evals = ITERATIONS)
		best = space_eval(space, best)
		accuracy = -hyperopt(best, X, y, new_dict)
...
def hyperopt(args, X, y, new_dict):
	...
	for train_index, test_index in skf.split(X, y):
		...
		if classifier == "Neural net":
			...
			accuracy = report_scoreNN(net, X_test, y_test, params, new_dict)
			res += [accuracy]
			continue
		...
		y_pred2 = y_pred.copy()
		for ind, _ in enumerate(y_pred):
			if y_test[ind] in new_dict[y_pred[ind]]:
				y_pred2[ind] = y_test[ind]
