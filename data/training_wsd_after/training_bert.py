...
def train_word(folder, bert_model, bert_tokenizer):
	...
	try:
		...
		le.fit(df["new_label"].values)
		df["new_label"] = le.transform(df["new_label"].values)
		...
		space = {'classifier': hp.choice('classifier',[
		        ...
		        'param':{ 
		            ...
		            'neurons_output':hp.choice('neurons_output', [len(np.unique(df["new_label"].values))]), 
		...
		y = df["new_label"].values.astype(int)
		...
		for sense in np.unique(df['sense'].values):
			...
			d['number_of_senses'] = len(set(df['new_label'].values))
